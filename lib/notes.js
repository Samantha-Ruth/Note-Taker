...
function filterByQuery(query, notesArray) {
    ...
    if (query.text_id) {
      filteredResults = filteredResults.filter(
        (notes) => notes.text_id === query.text_id
      );
    }
    ...
    if (title && text) {
      ...
      fs.readFile(`./data/db.json`, 'utf8', (err, data) => {
        if (err) {
          ...
          fs.writeFile(`./data/db.json`, JSON.stringify(parsedNote), (err) => err ? console.error(err) : console.log('Note entered and ID created.'))
        }
      });
